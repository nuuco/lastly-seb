...
**정확도** — 평가 문장 100개에서 의도·날짜·주기는 전량 일치, 이름 확정 100건.
같은 과제에서 호스팅 LLM 은 30문장 중 25건이었다. 답이 하나로 정해지는 문제라
규칙이 유리하고, 틀리면 한 줄 고쳐 테스트로 고정할 수 있다.

**행동만 사전으로 바꾸고 대상은 사용자가 말한 그대로 둔다.** 대상("가습기 필터",
"블라인드")은 끝이 없지만 행동은 유한하다. 넓힐 때는 `ACTION_NOUNS` 에 한 줄 더한다 —
동사(빨다·갈다·깎다)와 동작성 명사(설거지·분리수거·세차)를 모두 등록해야
`sawAction` 이 선다. 자세한 것은 [`@lastly/parser`](../../packages/parser/) 참고.
