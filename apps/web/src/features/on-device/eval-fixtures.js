...
exports.GOLDEN_KINDS = exports.GOLDEN_CASES = exports.GOLDEN_KNOWN_ITEMS = exports.GOLDEN_SIZE = exports.GOLDEN_REF_DATE = void 0;
exports.casesForKind = casesForKind;
exports.cadenceDays = cadenceDays;
 ...
 */
exports.GOLDEN_REF_DATE = '2026-09-14';
exports.GOLDEN_SIZE = 79;
exports.GOLDEN_KNOWN_ITEMS = [
...
function c(n, text, kind, trap, save, intent, itemName, daysAgo, cadence, matchId) {
...
if (exports.GOLDEN_CASES.length !== exports.GOLDEN_SIZE) {
    throw new Error(`골든셋 ${exports.GOLDEN_CASES.length}행. ${exports.GOLDEN_SIZE}여야 함`);
}
const KIND_COUNTS = {
    completed: 0,
    planned: 0,
...
for (const row of exports.GOLDEN_CASES)
    KIND_COUNTS[row.kind] += 1;
if (KIND_COUNTS.completed !== 26)
    throw new Error(`completed ${KIND_COUNTS.completed}행. 26여야 함`);
if (KIND_COUNTS.planned !== 15)
    throw new Error(`planned ${KIND_COUNTS.planned}행. 15여야 함`);
if (KIND_COUNTS.incomplete !== 15)
    throw new Error(`incomplete ${KIND_COUNTS.incomplete}행. 15여야 함`);
if (KIND_COUNTS.uncertain !== 15)
    throw new Error(`uncertain ${KIND_COUNTS.uncertain}행. 15여야 함`);
if (KIND_COUNTS.query !== 8)
    throw new Error(`query ${KIND_COUNTS.query}행. 8여야 함`);
exports.GOLDEN_KINDS = [
    { id: 'all', label: '전체 79' },
...
function casesForKind(kind) {
    return kind === 'all' ? exports.GOLDEN_CASES : exports.GOLDEN_CASES.filter((row) => row.kind === kind);
}
function cadenceDays(cadence) {
    if (cadence.kind === 'none')
        return null;
    if (cadence.kind === 'everyDays')
        return cadence.days;
    if (cadence.kind === 'everyMonths')
        return cadence.months * 30;
...
function cadenceLabel(cadence) {
    if (cadence.kind === 'none')
        return '없음';
    if (cadence.kind === 'everyDays')
        return `${cadence.days}일마다`;
    if (cadence.kind === 'everyMonths')
        return `${cadence.months}달마다`;
    if (cadence.kind === 'weekly')
        return `매주 ${cadence.weekday}`;
    if (cadence.kind === 'monthlyDay')
        return `매월 ${cadence.day}일`;
    if (cadence.nth === 'last')
        return `매월 마지막 ${cadence.weekday}`;
